...
public class VideoSkipParametersFrame extends JPanel
implements Parameters
{
    ...
    Properties setting;
    String fileName="video-skip-setting.properties";
	
//	AVGui avgui;
	public VideoSkipParametersFrame(){
//		this.avgui=g;
		this.initGUI();
	}
	...
	public int getInt(String x) {
		...
		if(x.equals("gap"))
			return this.gapBetweenDecreaseToIncrease;
		return 0;
	}
	/*
	double updateDecreasingFactor=2.0; //ì«Ç›îÚÇŒÇµä‘äuÇíZÇ≠Ç∑ÇÈäÑçá
		...
		try {
			{
				...
				});
				{
					jLabel1 = new JLabel();
					this.add(jLabel1);
					jLabel1.setText("Video Skip Setting");
	...
	public void saveProperties(){
		...
		this.setting.setProperty("skipIncFact", this.skipIncFactField.getText());
	       try {
	           ...
	           setting.store(saveS,"--- Video skip settings ---");
...
//	           System.err.println(e);
	        ...
	        try{
	        	int x=(new Integer(this.setting.getProperty("minimumInterval"))).intValue();
	            this.minimumInterval=x;
	            x=(new Integer(this.setting.getProperty("initialInterval"))).intValue();
    	        this.initialInterval=x;
	        	x=(new Integer(this.setting.getProperty("initialSkipterm"))).intValue();
	            this.initialSkipterm=x;
	            x=(new Integer(this.setting.getProperty("gap"))).intValue();
	            this.gapBetweenDecreaseToIncrease=x;
	            double d=(new Double(this.setting.getProperty("updDecFact"))).doubleValue();
