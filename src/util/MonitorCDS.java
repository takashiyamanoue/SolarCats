...
public class MonitorCDS extends PushBufferDataSource implements CaptureDevice {
	...
	private PushBufferDataSource audioDelegate = null;
	private PushBufferStream [] delStreams = null;
	private PushBufferStream [] audioStreams = null;
	private MonitorStream monitorStream = null;
	private MonitorStream audioStream = null;
...
public MonitorCDS(DataSource video, DataSource audio) {
// Get the stream from the actual datasource
// and create a MonitorStream from it
// Export the MonitorControl interface of the MonitorStream
if (video instanceof PushBufferDataSource &&
    audio instanceof PushBufferDataSource) {
	delegate = (PushBufferDataSource) video;
	delStreams = delegate.getStreams();
	audioDelegate = (PushBufferDataSource) audio;
	audioStreams = audioDelegate.getStreams();
	monitorStream = new MonitorStream(delStreams[0], this);
	audioStream = new MonitorStream(audioStreams[0], this);
	monitorStreams = new PushBufferStream[] {monitorStream, audioStream};
	controls = new Control[] {monitorStream};
}
}
